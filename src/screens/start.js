...
// 배경은 중심축 부감(BG-01). 그림에서 오려 낸 구름·새가 흐른다(요소정의 12절 · 구현을 보고 뺄 수 있다).
...
// 구름·새 조각 — 오려 낸 자리에서 출발해 오른쪽으로 흐르고, 끝에 닿으면 왼쪽에서 다시 들어온다
function drifter(p, kind) {
  const img = el(`img.thing.${kind}`, { src: imageUrl(p.file), alt: '' });
...
export async function startScreen(savedGame) {
  ...
  const sc = scene(cfg);
  for (const c of cfg.clouds || []) sc.append(drifter(c, 'cloud'));
  for (const b of cfg.birds || []) sc.append(drifter(b, 'bird'));
